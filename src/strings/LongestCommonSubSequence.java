        ...
        for (int i = 0; i < m + 1; i++) {
            for (int j = 0; j < n + 1; j++) {
                if (i == 0 || j == 0) {
                    ...
                    // removing the same character from both the string subsequence and adding 1 to the result of that.
                    // ex - AGGT - GXT -> removing T we get val(AGG - GX) + 1 which is in the diagonal box
                    dp[i][j] = 1 + dp[i-1][j-1];
                } else {
